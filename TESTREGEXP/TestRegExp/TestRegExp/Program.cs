...
namespace TestRegExp
{
    class Program
    {
         [STAThread] //indicates single threaded application
        static void Main(string[] args)
        {
            // Run this program on the command line.  The .exe is located here
            // C:\Users\wkkah\OneDrive\Walter\Programming Training\C#\Programs\First Cs Project\TESTREGEXP\TestRegExp\TestRegExp\bin\Debug>
            // Type this to test: TestRegExp ^\d{5}$ 12345
            
            //Regex.IsMatch returns true if string matches regular expression
            if (Regex.IsMatch(args[1],args[0]))
            // to add options Regex.IsMatch(expression, string, RegexOptions.Multiline) or use any of the options included on page 159
            {
